const StudentService = {
    init: function () {
        $('#addStudentForm').validate({
            rules: {
                name: 'required',
                email: {
                    required: true,
                    email: true
                },
                password: {
                    required: true,
                    minlength: 8,
                    maxlength: 20
                }
            },
            messages: {
                name: 'Please enter your name',
                email: {
                    required: 'Please enter your email',
                    email: 'Please enter a valid email address'
                },
                password: {
                    required: 'Please enter a password',
                    minlength: 'Password must be at least 8 characters long',
                    maxlength: 'Password cannot be longer than 20 characters',
                }
            },
            submitHandler: function (form) {
                let student = Object.fromEntries(new FormData(form).entries());
                StudentService.addStudent(student);
                form.reset();
            },
        });

        StudentService.getStudents();
    },

    addStudent: function (student) {
        try {
            $.blockUI({ message: '<h3>Processing...</h3>' });
            toastr.success("Student added successfully")
            $.unblockUI();
        } catch (error) {
            console.error('Error in submitHandler:', error);
            alert('An error occurred. Please try again later.');
            $.unblockUI();
        }
    },

    getStudents : function () {
        $.ajax({
            url: '/fenms035-lecture - priprema/data/students.json', // Replace 'data.json' with the path to your JSON file
            type: 'GET',
            dataType: 'json',
            success: function (data) {
                console.log(data)
               /*let html = "<tr><th>Name</th><th>Email</th><th>Password</th><th>Actions</th></tr>";
                data.forEach(element => {
                    html+='<tr><td>'+element.name+'</td><td>'+element.email+'</td><td>'+element.password+'</td>'+
                          '<td><button onclick=StudentService.openEditModal('+element.id+')>Edit</button></td></tr>';
                });
                $("table#students").html(html)*/         
                $("table#students").DataTable({
                    data: data, 
                    columns: [
                        { data: 'name', title: 'Name' },
                        { data: 'email', title: 'Email' }, 
                        { data: 'password', title: 'Password' },
                        {
                        title: 'Action',
                            render: function (data, type, row, meta) {
                                return '<button class="btn btn-primary" onclick="StudentService.openEditModal(\'' + row.id + '\')">Edit Student</button>'+
                                       '<button class="btn btn-primary" onclick="StudentService.viewMore(\'' + row.id + '\')">View More</button>';
                            }
                        }
                    ]
                })
                /*
                Utils.datatable(
                    "students-table",
                    [
                        { data: "name", title: "Name" },                        
                        { data: "email", title: "Email" },
                        { data: "password", title: "Password" },
                        { data: "edit_student", title: "Edit Student" }
                    ],
                    data
                );*/
                console.log('Data from file:', data);
            },
            error: function (xhr, status, error) {
                console.error('Error fetching data from file:', error);
            }
        });
    },

    getStudentById : function(id) {
        RestClient.get('data/student_' + id + '.json', function(data){
            console.log('rest')
            console.log('Data for student ' + id)
            console.log(data)
            $('input[name="name"]').val(data.name)
            $('input[name="email"]').val(data.email)
            $('input[name="password"]').val(data.password)

            localStorage.setItem("selected_student", JSON.stringify(data));
            
            $.unblockUI(); 
        }, function(xhr){
            console.error('Error fetching data from file:', error);
                $.unblockUI();
        });
        /*$.ajax({
            url: '/fenms035-lecture - priprema/data/student_' + id + '.json',
            type: 'GET',
            dataType: 'json',
            success: function (data) {
                console.log('Data for student ' + id)
                console.log(data)
                $('input[name="name"]').val(data.name)
                $('input[name="email"]').val(data.email)
                $('input[name="password"]').val(data.password)
                
                $.unblockUI();
            },
            error: function (xhr, status, error) {
                console.error('Error fetching data from file:', error);
                $.unblockUI();
            }
        });*/
    },

    openEditModal : function (id) {
        console.log(id)
        toastr.success('You clicked Success toast');
        $.blockUI({ message: '<h3>Processing...</h3>' });
        $('#editModal').show();
        StudentService.getStudentById(id)   
    },

    viewMore : function (id) {
        StudentService.getStudentById(id) 
        window.location.replace("#view_more");
    },

    populateViewMore : function(){
        let student = JSON.parse(localStorage.getItem('selected_student'))        
        $("#name").text(student.name)
        $("#email").text(student.email)

    },

    closeModal : function() {
        $("#editModal").hide();
    },

    editStudent : function(student){
        try {
            $.blockUI({ message: '<h3>Processing...</h3>' });
            toastr.success("Success");
            $.unblockUI();
        } catch (error) {
            console.error('Error in submitHandler:', error);
            alert('An error occurred. Please try again later.');
            $.unblockUI();
        }
    }
};